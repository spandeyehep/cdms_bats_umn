...
while($line = <RUNLIST>) {
 ...
 if($runid =~ m/$searchR130/||$runid =~ m/$searchR131/||$runid =~ m/$searchR133/)
 {
  ...
  #run BatCalib scripts
  print "\nCalibrating the series $series\n";

  $ENV{'CDMSBATSDIR'} = '/micro/data3/cdmsbatsProd/kludgepicoscripts/cdmsbats';
  $ENV{'BATCALIB_RQDATA'} = '/micro/data3/cdmsbatsProd/'.$runid.'/dataReleases/'.$prefix.'/merged/all/'.$datatype;
  $ENV{'BATCALIB_RRQDATA'} = '/micro/data3/cdmsbatsProd/'.$runid.'/dataReleases/'.$prefix.'/merged/all/'.$datatype;
  system("printenv | grep BATCALIB; $ENV{'CDMSBATSDIR'}/BUILD/bin/BatCalib $series merged 1001000 optionsSoudanData.SuperCDMS >logs/calib\_$runlist\_$series.log 2>logs/calib\_$runlist\_$series.err;");
