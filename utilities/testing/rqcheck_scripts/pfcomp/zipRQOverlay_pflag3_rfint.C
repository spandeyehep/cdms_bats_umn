void doOverlay(int t, int z, string chan, string chanUp, 
	       string brRQ, string pfRQ, string pfRQ2, string filename, int dumpNum, string brPulseFlag, string pfPulseFlag,string rfeflag);

void zipRQOverlay_pflag3_rfint(string brRQ, string pfRQ, string pfRQ2, string brPulseFlag, string pfPulseFlag, string rfeflag,string filename="170319_1616_F0006", int dumpNum=6, int notCharge=0)
{
   ...
   for(int tCtr = 1; tCtr <= 5; tCtr++)
   {
	 ...
	 if(!notCharge==0)
	 {
	   doOverlay(tCtr, zCtr, "qi", "QI", brRQ, pfRQ, pfRQ2, filename, dumpNum,brPulseFlag,pfPulseFlag,rfeflag);
	   doOverlay(tCtr, zCtr, "qo", "QO", brRQ, pfRQ, pfRQ2, filename, dumpNum,brPulseFlag,pfPUlseFlag,rfeflag);
	 }

  	 doOverlay(tCtr, zCtr, "pa", "PA", brRQ, pfRQ, pfRQ2, filename, dumpNum,brPulseFlag,pfPulseFlag,rfeflag);
  	 doOverlay(tCtr, zCtr, "pb", "PB", brRQ, pfRQ, pfRQ2, filename, dumpNum,brPulseFlag,pfPulseFlag,rfeflag);
  	 doOverlay(tCtr, zCtr, "pc", "PC", brRQ, pfRQ, pfRQ2, filename, dumpNum,brPulseFlag,pfPulseFlag,rfeflag);
  	 doOverlay(tCtr, zCtr, "pd", "PD", brRQ, pfRQ, pfRQ2, filename, dumpNum,brPulseFlag,pfPulseFlag,rfeflag);
	       ...
	       string brRQ, string pfRQ, string pfRQ2, string filename, int dumpNum, string brPulseFlag, string pfPulseFlag, string rfeflag)
{
   ...
   TString brweight = Form("(%s%s == 3 && %s%s == 0)*(%s%s)", chanUp.c_str(), brPulseFlag.c_str(), chanUp.c_str(), rfeflag.c_str(),chanUp.c_str(),brRQ.c_str());
   TString brplot = Form("eventTree.EventNumber>>hbr_event(500, %d0000, %d0500)", dumpNum, dumpNum); 
   brTree->Draw(brplot, brweight);

   //make cut on pulse flag
   TString prweight = Form("(%s.%s == 3 && %s.%s > -999)*(%s.%s)", chan.c_str(), pfPulseFlag.c_str(),chan.c_str(), pfRQ2.c_str() ,chan.c_str(), pfRQ.c_str());
   ...
   prTree->Draw(prplot, prweight); 
     ...
     mycan->cd(1);
     TLatex* label1 = new TLatex(0.53, 0.9, Form("PulseFlag: 3 zip%d %s: PipeRoot over BatRoot", zip, chanUp));
     label1->SetNDC();
     ...
     mycan->cd(2);
     TLatex* label2 = new TLatex(0.53, 0.9, Form("PulseFlag: 3 zip%d %s: BatRoot over Darkpipe", zip, chanUp));
     label2->SetNDC();
     ...
     mycan->cd(3);
     TLatex* label3 = new TLatex(0.55, 0.9, Form("PulseFlag: 3 zip %d %s: %s", zip, chanUp.c_str(), filename.c_str()));
     label3->SetNDC();
     ...
     mycan->Print(Form("plots_pflag3/pfbrpf3_%sover_t%dz%d_%s.gif", brRQ.c_str(), t, z, chanUp.c_str())); 
