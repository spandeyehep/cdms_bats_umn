void doOverlay(int t, int z, string chan, string chanUp, 
	       string brRQ, string pfRQ, string filename, int dumpNum, string brPulseFlag, string pfPulseFlag,string rfeflag);

void zipRQOverlay_pflag3_rfcheck(string brRQ, string pfRQ, string brPulseFlag, string pfPulseFlag, string rfeflag,string filename="170319_1616_F0006", int dumpNum=6, int notCharge=0)
{
   ...
   for(int tCtr = 1; tCtr <= 5; tCtr++)
   {
	 ...
	 if(!notCharge==0)
	 {
	   doOverlay(tCtr, zCtr, "qi", "QI", brRQ, pfRQ, filename, dumpNum,brPulseFlag,pfPulseFlag,rfeflag);
	   doOverlay(tCtr, zCtr, "qo", "QO", brRQ, pfRQ, filename, dumpNum,brPulseFlag,pfPUlseFlag,rfeflag);
	 }

  	 doOverlay(tCtr, zCtr, "pa", "PA", brRQ, pfRQ, filename, dumpNum,brPulseFlag,pfPulseFlag,rfeflag);
  	 doOverlay(tCtr, zCtr, "pb", "PB", brRQ, pfRQ, filename, dumpNum,brPulseFlag,pfPulseFlag,rfeflag);
  	 doOverlay(tCtr, zCtr, "pc", "PC", brRQ, pfRQ, filename, dumpNum,brPulseFlag,pfPulseFlag,rfeflag);
  	 doOverlay(tCtr, zCtr, "pd", "PD", brRQ, pfRQ, filename, dumpNum,brPulseFlag,pfPulseFlag,rfeflag);
	       ...
	       string brRQ, string pfRQ, string filename, int dumpNum, string brPulseFlag, string pfPulseFlag, string rfeflag)
{
   ...
   //BatRoot file   
   TFile f_br(Form("/home/kos/cdmsbats/BatRoot/test_%s.root", filename.c_str()));
   ...
   //make cut on pulse flag
   TString prweight = Form("(%s.%s == 3 && %s.%s > -999)*(%s.%s)", chan.c_str(), pfPulseFlag.c_str(),chan.c_str(),pfRQ.c_str() ,chan.c_str(), pfRQ.c_str());
