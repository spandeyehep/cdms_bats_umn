...
void zipRQOverlay_isnotnoise(string brRQ, string prRQ, string filename="170319_1616_F0006", int dumpNum=6, int notCharge=0)
{
	       ...
	       string brRQ, string prRQ, string filename, int dumpNum)
{
   ...
   //PipeRoot file
   TFile f_pr(Form("/home/kos/PipeFitter/piperoot/%s_notrig.root.t%dz%d", filename, t, z));
   TTree* prTree = f_pr.Get(Form("t%dz%d",t,z));

   //Make histograms
   TString brweight = Form("(%s%s == 0)", chanUp.c_str(), brRQ.c_str(), chanUp.c_str(), brRQ.c_str());
   TString brplot = Form("eventTree.EventNumber>>hbr_event(500, %d0000, %d0500)", dumpNum, dumpNum); 
   brTree->Draw(brplot, brweight);

   TString prweight = Form("(%s.%s > 0)", chan.c_str(), prRQ.c_str(), chan.c_str(), prRQ.c_str());
   ...
   hbr_event->SetYTitle(Form("not%s", brRQ.c_str()));
   ...
   hpr_event->SetYTitle(Form("not%s", brRQ.c_str()));
   ...
   mycan->Print(Form("plots/prbr_not%sover_t%dz%d_%s.gif", brRQ.c_str(), t, z, chanUp.c_str())); 
