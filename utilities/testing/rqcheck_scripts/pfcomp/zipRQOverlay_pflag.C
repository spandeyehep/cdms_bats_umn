...
void zipRQOverlay_pflag(string brRQ, string prRQ, string filename="170319_1616_F0006", int dumpNum=6, int notCharge=0)
{
	       ...
	       string brRQ, string prRQ, string filename, int dumpNum)
{
   ...
   //Make histograms
   TString brweight = Form("(%s%s != -999999.)*(%s%s)", chanUp.c_str(), brRQ.c_str(), chanUp.c_str(), brRQ.c_str());
   TString brplot = Form("eventTree.EventNumber>>hbr_event(500, %d0000, %d0500)", dumpNum, dumpNum); 
   brTree->Draw(brplot, brweight);

   TString prweight = Form("(%s.%s > 0.)*(%s.%s)", chan.c_str(), prRQ.c_str(), chan.c_str(), prRQ.c_str());
