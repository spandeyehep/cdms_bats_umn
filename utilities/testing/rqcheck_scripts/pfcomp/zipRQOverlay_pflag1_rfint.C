...
void zipRQOverlay_pflag1_rfint(string brRQ, string pfRQ, string pfRQ2, string brPulseFlag, string pfPulseFlag, string rfeflag,string filename="170319_1616_F0006", int dumpNum=6, int notCharge=0)
{
	       ...
	       string brRQ, string pfRQ, string pfRQ2, string filename, int dumpNum, string brPulseFlag, string pfPulseFlag, string rfeflag)
{
   ...
   TString brweight = Form("(%s%s == 1 && %s%s == 0)*(%s%s)", chanUp.c_str(), brPulseFlag.c_str(), chanUp.c_str(), rfeflag.c_str(),chanUp.c_str(),brRQ.c_str());
   TString brplot = Form("eventTree.EventNumber>>hbr_event(500, %d0000, %d0500)", dumpNum, dumpNum); 
   brTree->Draw(brplot, brweight);

   //make cut on pulse flag
   TString prweight = Form("(%s.%s == 1 && %s.%s > -999)*(%s.%s)", chan.c_str(), pfPulseFlag.c_str(),chan.c_str(), pfRQ2.c_str() ,chan.c_str(), pfRQ.c_str());
