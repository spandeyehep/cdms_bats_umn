void SeriesOverlay()
{
   ...
   //Make histograms
   brTree->Draw("EventNumber>>hbr_event(500, 60000, 60500)", "SeriesNumber");
   prTree->Draw("event>>hpr_event(500, 60000, 60500)", "series"); 
   ...
   hpr_event->SetFillColor(kBlue);   
    ...
    h_diff->SetTitle("");
    h_diff->SetYTitle("Series fractional diff");
    h_diff->SetXTitle("eventNumber");
    h_diff->DrawCopy();

    mycan->Print("plots/Series.gif");
