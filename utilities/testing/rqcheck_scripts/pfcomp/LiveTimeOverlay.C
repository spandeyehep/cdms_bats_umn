void LiveTimeOverlay()
{
   ...
   //Make histograms
   brTree->Draw("EventNumber>>hbr_event(500, 60000, 60500)", "LiveTime");
   prTree->Draw("event>>hpr_event(500, 60000, 60500)", "ltimelev"); 
   ...
   hpr_event->SetFillColor(kBlue);   
    ...
    h_diff->SetTitle("");
    h_diff->SetYTitle("LiveTime fractional diff");
    h_diff->SetXTitle("eventNumber");
    h_diff->DrawCopy();

    mycan->Print("plots/LiveTime.gif");
