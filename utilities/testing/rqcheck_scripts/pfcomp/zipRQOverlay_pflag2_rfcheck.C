...
void zipRQOverlay_pflag2_rfcheck(string brRQ, string pfRQ, string brPulseFlag, string pfPulseFlag, string rfeflag,string filename="170319_1616_F0006", int dumpNum=6, int notCharge=0)
{
	       ...
	       string brRQ, string pfRQ, string filename, int dumpNum, string brPulseFlag, string pfPulseFlag, string rfeflag)
{
   ...
   //make cut on pulse flag
   TString prweight = Form("(%s.%s == 2 && %s.%s > -999)*(%s.%s)", chan.c_str(), pfPulseFlag.c_str(),chan.c_str(),pfRQ.c_str() ,chan.c_str(), pfRQ.c_str());
