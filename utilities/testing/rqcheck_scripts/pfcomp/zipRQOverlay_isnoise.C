...
void zipRQOverlay_isnoise(string brRQ, string prRQ, string filename="170319_1616_F0006", int dumpNum=6, int notCharge=0)
{
	       ...
	       string brRQ, string prRQ, string filename, int dumpNum)
{
   ...
   //Make histograms
   TString brweight = Form("(%s%s == 1)", chanUp.c_str(), brRQ.c_str(), chanUp.c_str(), brRQ.c_str());
   TString brplot = Form("eventTree.EventNumber>>hbr_event(500, %d0000, %d0500)", dumpNum, dumpNum); 
   brTree->Draw(brplot, brweight);

   TString prweight = Form("(%s.%s == -1)", chan.c_str(), prRQ.c_str(), chan.c_str(), prRQ.c_str());
