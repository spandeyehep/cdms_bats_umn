void EventTimeOverlay()
{
   ...
   //Make histograms
   brTree->Draw("EventNumber>>hbr_event(500, 60000, 60500)", "EventTime");
   prTree->Draw("event>>hpr_event(500, 60000, 60500)", "evtime"); 
   ...
   hpr_event->SetFillColor(kBlue);   
    ...
    h_diff->SetTitle("");
    h_diff->SetYTitle("EventTime fractional diff");
    h_diff->SetXTitle("eventNumber");
    h_diff->DrawCopy();

    mycan->Print("plots/EventTime.gif");
