...
void zipRQOverlay_pflag2_rfint(string brRQ, string pfRQ, string pfRQ2, string brPulseFlag, string pfPulseFlag, string rfeflag,string filename="170319_1616_F0006", int dumpNum=6, int notCharge=0)
{
	       ...
	       string brRQ, string pfRQ, string pfRQ2, string filename, int dumpNum, string brPulseFlag, string pfPulseFlag, string rfeflag)
{
   ...
   TString brweight = Form("(%s%s == 2 && %s%s == 0)*(%s%s)", chanUp.c_str(), brPulseFlag.c_str(), chanUp.c_str(), rfeflag.c_str(),chanUp.c_str(),brRQ.c_str());
   TString brplot = Form("eventTree.EventNumber>>hbr_event(500, %d0000, %d0500)", dumpNum, dumpNum); 
   brTree->Draw(brplot, brweight);

   //make cut on pulse flag
   TString prweight = Form("(%s.%s == 2 && %s.%s > -999)*(%s.%s)", chan.c_str(), pfPulseFlag.c_str(),chan.c_str(), pfRQ2.c_str() ,chan.c_str(), pfRQ.c_str());
   ...
   prTree->Draw(prplot, prweight); 
     ...
     mycan->cd(1);
     TLatex* label1 = new TLatex(0.53, 0.9, Form("PulseFlag: 2 zip%d %s: PipeRoot over BatRoot", zip, chanUp));
     label1->SetNDC();
     ...
     mycan->cd(2);
     TLatex* label2 = new TLatex(0.53, 0.9, Form("PulseFlag: 2 zip%d %s: BatRoot over Darkpipe", zip, chanUp));
     label2->SetNDC();
     ...
     mycan->cd(3);
     TLatex* label3 = new TLatex(0.55, 0.9, Form("PulseFlag: 2 zip %d %s: %s", zip, chanUp.c_str(), filename.c_str()));
     label3->SetNDC();
     ...
     mycan->Print(Form("plots_pflag2/pfbrpf2_%sover_t%dz%d_%s.gif", brRQ.c_str(), t, z, chanUp.c_str())); 
