...
//default constructor
EventBuilder::EventBuilder(UserDataManager& myUserData, DetectorConfigManager& myDetectorConfigManager,
...
{
   //cout <<"Constructing EventBuilder" << endl;
   ...
   fRawReader.RegisterNoiseMonitorPulseVector(&fVectorOfNoiseMonitorPulses); //will read out all noise monitor pulses
   ...
   if (fUserData.DoModifyRawData()) {
      ...
      if (!modificationMap.empty())
        ...
        cerr <<"EventBuilder::ERROR!  Cannot modify raw data without modification type!"  
...
uint32_t EventBuilder::GetEventCategory()
{
  return fRawReader.GetEventCategory();
}

int EventBuilder::ReadNextEvent()
{
...
int EventBuilder::ReadEventN(int eventN)
{
   //First clear data containers of previous event's data
   fRawReader.Clear();
...
void EventBuilder::ResetDataReader()
{
   //Reset the raw data reader!
...
void EventBuilder::SetSimDataManager(string input_filename)
{
...
void EventBuilder::ReadSimEvent()
{
    fSimDataReader.readEvent();
}


void EventBuilder::SetPulseLibManager(vector<string> filename)
{
    map<int, int> detectorMap = fDetectorConfigManager.GetDetectorMap();
    cout << "    DETECOR MAP :: " << endl;
    for(map<int, int>::const_iterator it = detectorMap.begin();
	it != detectorMap.end(); ++it)
      {
...
void EventBuilder::DoBasicPulseCalc(int detNum)
{
    ...
    if(mapItr != fMapOfZipPulses.end())
    {
	  ...
	  if(fUserData.HasIntParameter(detNum, parNameBase+ "_DO_BASELINE_SLOPED_SUB")){
	    tempBasicPulseCalc.SetSlopedBaselineSub(fUserData.GetIntParameter(detNum, parNameBase+ "_DO_BASELINE_SLOPED_SUB"));
	  }
	  ...
	  if(phononSumPulse.size() == 0)
	  {
       ...
       phononSumPulseData.fIsNoiseMonitor = false;
	 ...
	 phononSumS1PulseData.fIsNoiseMonitor = false;
	 ...
	 phononSumS2PulseData.fIsNoiseMonitor = false;
...
void EventBuilder::DoNoiseSelector(int detNum, const string& sensorType)
{
...
void EventBuilder::DoVarFreqRTFTWalkPhonon(int detNum, const string& sensorType, const string& pulseType)
{
...
void EventBuilder::DoRTFTWalkCharge(int detNum, const string& sensorType, const string& pulseType)
{
   //retrieve the vector of pulses for this zip
   vector<PulseData>* zipPulseList;
   map< int, vector<PulseData> >::iterator mapItr = fMapOfZipPulses.find(detNum);
...
void EventBuilder::DoConstFreqRTFTWalkPhonon(int detNum, const string& sensorType,  const string& pulseType)
{
    //retreive the vector of pulses for this zip
    vector<PulseData>* zipPulseList;
    map< int, vector<PulseData> >::iterator mapItr = fMapOfZipPulses.find(detNum);
...
void EventBuilder::DoPulseIntegral(int detNum, const string& sensorType, const string& pulseType)
{
...
void EventBuilder::DoTailFitPhonon(int detNum, const string& sensorType)
{
        ...
        if (fUserData.GetIntParameter(detNum,"P_TAIL_FIT_USE_QDELAY")==1) 
         {
              ...
              if (fUserData.DoAlgorithm(detNum,"charge","OptimalFilterCharge2X2"))
                ...
                cout <<"\nERROR! EventBuilder::DoTailFitPhonon: No charge OF delay available."
...
void EventBuilder::DoSimulatePhononFromRandoms(int detNum)
{
...
void EventBuilder::DoSimulateFromPulse(int detNum, vector<string> filename, int thEvt)
{
    //skip if not a random triggered event
    //BatRoot main should take care of this check, 
    //repeating here just in case
    ...
    // do nothing if the pulse simulation is not enabled
    if(fUserData.GetIntParameter("DO_SIM_FROM_PULSE") != 1)
	return;	

    // get the event energies
    map<string, double> eventEnergies = fSimDataReader.getEventEnergies(detNum);
    int libNum = fSimDataReader.getLibNum(detNum);
    if(libNum >= filename.size())
	std::cout << "ERROR in EventBuilder::DoSimulateFromPulse: "
		  << "Energy input file is requesting a library number that is not defined!" << std::endl;
    ...
    if(thEvt<0)
      fSimLibManager.readRandomEvent(detNum, filename[libNum]);
    else
      fSimLibManager.readEvent(thEvt, detNum, filename[libNum]);
    map<string, vector<double>* > pulseMap = fSimLibManager.getPulseMap(detNum);
    map<string, double> chanAmp = fSimLibManager.getChanAmps();
    double eventNum = fSimLibManager.getEventNum();
    double seriesNum = fSimLibManager.getSeriesNum();
    double DMCXPosition = fSimLibManager.getDMCXPosition();
    double DMCYPosition = fSimLibManager.getDMCYPosition();
    double DMCZPosition = fSimLibManager.getDMCZPosition();
    double DMCRecoilEnergy = fSimLibManager.getDMCRecoilEnergy();
	...
	{	  
	    ...
	    if(detType != BatRootTypes::kiZIPSoudan && detType != BatRootTypes::kCDMSliteSoudanI)
	    {
		std::cerr << "WARNING: You are attempting to run pulse simulation on a detector type that is"
			  << " neither a Soudan iZIP nor CDMSlite. Skipping simulation for channel " 
			  << chanName << "!!" << std::endl;
		continue;
	    }
		...
		continue;
	    if(fUserData.GetIntParameter("DO_PCHANSIM") != 1 && ChannelMapHelper::IsPhysicalChannel(chanName) && 
	       chanName[0] == 'P')
		continue;
	    ...
	    // Get the trace (should be a random) (aPulse = Noise Event)
	    vector<double> aPulse = aPulseData->GetBaselineSubNormPulse();

	    if(fUserData.GetIntParameter("SIM_NO_NOISE") == 1)
	    {	
	      for(int binItr = 0; binItr < aPulse.size(); binItr++)
	      {
		// Kill noise to zero. Workaround to get DMC pulses without noise processed.
		aPulse[binItr]=0*aPulse[binItr];
	      }
	    }
	    // get the calibration for this channel
	    double chanCal;
		...
		std::cout << "ERROR in EventBuilder::DoSimulateFromPulse: channel name is neither phonon nor charge!!" << std::endl;
	    
            // inject the pulse
	    tempSimulateFromRandoms.SimPwDataTemplate(aPulse, *(pulseMap[chanName]), eventEnergies["PT"], 
						      chanCal, chanAmp, chanName, libNum, eventNum, seriesNum, 
						      DMCXPosition, DMCYPosition, DMCZPosition, DMCRecoilEnergy);
...
void EventBuilder::DoSimulateChargeFromRandoms(int detNum)
{
  //skip if not a random triggered event
  //BatRoot main should take care of this check, 
  //repeating here just in case
...
//Modify this as needed to correctly call your class
void EventBuilder::DoOptimalFilterPhonon1X2(int detNum, const string& sensorType)
{
    ...
    if(mapItr != fMapOfZipPulses.end())
    {
       ...
       {	  
           ...
           if(aPulseData->GetChannelName() == "PT" && !fUserData.DoAlgorithm(detNum,"PT","OptimalFilterPhonon1X2")) { continue; }
           if(aPulseData->GetChannelName() == "PS1" && !fUserData.DoAlgorithm(detNum,"PSIDES","OptimalFilterPhonon1X2")) { continue; }
           if(aPulseData->GetChannelName() == "PS2" && !fUserData.DoAlgorithm(detNum,"PSIDES","OptimalFilterPhonon1X2")) { continue; }
           
           // ------- getting templates for OF ---------
           
	   string chanNameSlow = chanName + "slow";
           vector<double>   templateSlow     = fFilterData.GetTemplateTime(detNum,chanNameSlow);
	   vector<TComplex> templateSlowFFT  = fFilterData.GetTemplateFFT(detNum,chanNameSlow);

	   string chanNameFast = chanName + "fast";
	   vector<double>   templateFast   = fFilterData.GetTemplateTime(detNum,chanNameFast);//Change once we have the residuals template
           vector<TComplex> templateFastFFT  = fFilterData.GetTemplateFFT(detNum,chanNameFast);
           ...
           OptimalFilterPhonon1X2 tempOptimalFilterPhonon1X2;

	    //set timing parameters
           tempOptimalFilterPhonon1X2.SetSampleTime(1.0/sampleRate);
           tempOptimalFilterPhonon1X2.SetPxWindows(win1,win2);
           
           tempOptimalFilterPhonon1X2.LoadTemplates(templateSlow,templateFast,templateSlowFFT,templateFastFFT);
           tempOptimalFilterPhonon1X2.LoadNormalizations(noiseFFTsq);

           // 
	   tempOptimalFilterPhonon1X2.SetDelayInterpolateFlag(fUserData.GetIntParameter(detNum, "P_OF1X2_DELAY_INTERPOLATE"));
          
           
           // -------  do Optimal Filter ------------
           //Get the pulse
           //other options are: GetRawPulse, GetBaselineSubPulse 
           vector<double> aPulse = aPulseData->GetBaselineSubNormPulse();//(norm = 1 when isr is not read)
           
           
           tempOptimalFilterPhonon1X2.DoCalc(aPulse);
           
           //store instance of this class so RQs can be read out a little later
           aPulseData->StorePulseAnalysis(tempOptimalFilterPhonon1X2);
       }
...
//Modify this as needed to correctly call your class
void EventBuilder::DoOptimalFilterPhononNS(int detNum, const string& sensorType)
{
...
void EventBuilder::DoOptimalFilterCharge2X2(int detNum)
{
    ...
    // ====================================
    // Determine if any pulse saturated
    // ===================================
    ...
    // staturation flag, if true only single OF on non
    // saturated pulse will be done
    bool isAnyPulseSaturated = false;
	...
	if(detType != BatRootTypes::kZIPDetType &&
	   ...
	   detType != BatRootTypes::kiZIPSNOlab)
	  {
	    cout <<"\nERROR! EventBuilder::DoOptimalFilterCharge2X2  OptimalFilterCharge2X2 calculation "
	...
	//  no optimal filter on saturated pulse
        if (aPulseData->GetPulseAnalysis("BasicPulseCalc").GetRQVal("sat") != 0)
	  continue; // no optimal filter on that pulse
    ...
    // No Z time constraint if single pulse fitting 
    ...
    // NOTES: 
    // 
    // (1) In the UserSettings file, Q_PEAK_WINDOW_MIN/MAX are always positive
    //
    // so if shiftPT = 0 => 
    //  OF window = [- sampleRate * Q_PEAK_WINDOW_MIN :  + sampleRate * Q_PEAK_WINDOW_MAX]
    //  
    // (2) Conversion  to all positive values (cyclical window: [0 : traceLength-1]) is done in OptimalFilterNxN
    //   (ie for example,  qxwinMin = -1 -> qxwinMin = traceLength-1)
   ...
   if (aPulseMap1x1.size()>0) {
	 ...
	 double templateMax = fFilterData.GetTemplateMax(detNum,chanName);
         ...
         // Windows
         myOptimalFilterCharge1x1.SetSampleTime(1.0/sampleRate);//This is used in normalizations and so must come first  	    
...
//Modify this as needed to correctly call your class
void EventBuilder::DoPSDIntegralPhonon(int detNum, const string& sensorType)
{
...
void EventBuilder::DoWedgeFitPhonon(int detNum, const string& sensorType)
{
...
//Modify this as needed to correctly call your class
void EventBuilder::DoVetoAnalysis()
{
    //some quantities need history information, so check if trigger processsing activated
    if(!fUserData.DoTriggerProcessing())
    {
       cerr <<"EventBuilder::ERROR!  Veto Analysis requires history trigger information!"  
...
//Modify this as needed to correctly call your class
void EventBuilder::DoNoiseMonitorAnalysis()
{
  //loop over entire pulse collection
    for(uint pulseItr = 0; pulseItr < fVectorOfNoiseMonitorPulses.size(); pulseItr++)
    {
      PulseData*     aPulseData = &fVectorOfNoiseMonitorPulses[pulseItr]; 
      if(!fUserData.DoNoiseMonitorProcessing())
	continue;
    
       NoiseMonitorAnalysis     tempNoiseMonitorAnalysis;

       //optional function for setting parameters         
       tempNoiseMonitorAnalysis.InitializeParameters();
       //setting functions
       
       //only raw pulses are filled for noise monitors
       tempNoiseMonitorAnalysis.DoCalc(aPulseData->GetRawPulse());

       //store instance of this class so RQs can be read out a little later
       aPulseData->StorePulseAnalysis(tempNoiseMonitorAnalysis);
     }
...
void EventBuilder::DoInflectionTime(int detNum, const string& sensorType)
{ 
   //retrive the vector of pulses for this zip
   vector<PulseData>* zipPulseList;
   map< int, vector<PulseData> >::iterator mapItr = fMapOfZipPulses.find(detNum);
...
void EventBuilder::DoPipeFitPhonon(int detNum, const string& sensorType)
{
   //retrive the vector of pulses for this zip
   vector<PulseData>* zipPulseList;
   map< int, vector<PulseData> >::iterator mapItr = fMapOfZipPulses.find(detNum);
...
void EventBuilder::DoOptimalFilterPhonon(int detNum, const string& sensorType)
{
   //retrive the vector of pulses for this zip
   vector<PulseData>* zipPulseList;
   map< int, vector<PulseData> >::iterator mapItr = fMapOfZipPulses.find(detNum);
...
void EventBuilder::DoOptimalFilterPhononDMC(int detNum, const string& sensorType)
{
   //retrive the vector of pulses for this zip
   vector<PulseData>* zipPulseList;
   map< int, vector<PulseData> >::iterator mapItr = fMapOfZipPulses.find(detNum);
   if(mapItr != fMapOfZipPulses.end())
   {
	 ...
	 PulseData* aPulseData = &((*zipPulseList)[pulseItr]); 
	 string chanName   = aPulseData->GetChannelName(); 
	 string chanNameDMC = chanName+ "dmc";
	 int    detCode    = aPulseData->GetDetectorCode();
	 
	 if(aPulseData->GetChannelType() != sensorType) { continue; } 
	 
         // check if analysis flag for sum of phonon pulses
         if(chanName == "PT" && !fUserData.DoAlgorithm(detNum,"PT","OptimalFilterPhononDMC")) {continue; }
         if(chanName == "PS1" && !fUserData.DoAlgorithm(detNum,"PSIDES","OptimalFilterPhononDMC")) {continue; }
         if(chanName == "PS2" && !fUserData.DoAlgorithm(detNum,"PSIDES","OptimalFilterPhononDMC")) {continue; }
	 
	 // ------- getting templates for OF ---------
	 
	 vector<TComplex> templateConjNoiseFFT = fFilterData.GetTemplateConjNoiseFFT(detNum,chanNameDMC);
	 vector<TComplex> templateFFT = fFilterData.GetTemplateFFT(detNum,chanNameDMC);
	 
	
         
         // ------- getting normalization for OF ---------

	 vector<double> noiseFFTsq  = fFilterData.GetNoiseFFTsq(detNum,chanNameDMC);
	 double normFFT = fFilterData.GetNormFFT(detNum,chanNameDMC);
	 double sigToNoiseSq = fFilterData.GetSigToNoiseSq(detNum,chanNameDMC);
	 ...
	 int win2 = windowMin + postTriggerBins + 1;
         ...
         OptimalFilterPhonon tempOptimalFilterPhonon("OptimalFilterPhononDMC");
...
void EventBuilder::DoOptimalFilterPhononGlitch1(int detNum, const string& sensorType)
{
...
void EventBuilder::DoOptimalFilterPhononLFnoise1(int detNum, const string& sensorType)
{
...
void EventBuilder::DoOptimalFilterCharge(int detNum)
{
   ...
   if(mapItr != fMapOfZipPulses.end())
   {
	 ...
	 // ------   Get OptimalFilterCharge parameters ---------
	 ...
	 OptimalFilterNxN myOptimalFilterCharge("OptimalFilterCharge");
	 
         //set timing parameters
	 myOptimalFilterCharge.SetSampleTime(1.0/sampleRate);//This must come first since it's used in normalizations
	 myOptimalFilterCharge.SetXwindows(qxwinMin, qxwinMax,traceLength); 
	 
         //Load Normalizations	 
	 myOptimalFilterCharge.LoadTemplates(templateFFT, templateConjNoiseFFT,chanName); 
	 myOptimalFilterCharge.LoadNormalizations(sigToNoiseSq, noiseFFTsq, templateMax, chanName); 
	
	 // Do the delay interpolation?
         myOptimalFilterCharge.SetDelayInterpolateFlag(fUserData.GetIntParameter(detNum, "Q_DELAY_INTERPOLATE"));
	 
	 // --------- Do Optimal Filter ---------------
	 myOptimalFilterCharge.DoCalc(aPulseData->GetBaselineSubNormPulse(),chanName); 
	 
	 
	 // ---------- store results -------------------
     	 myOptimalFilterCharge.StoreAs(chanName);
	 aPulseData->StorePulseAnalysis(myOptimalFilterCharge);
...
void EventBuilder::DoOptimalFilterChargeX(int detNum, const string& side)
{ 
   ...
   if (side == "S1" || side == "S2") {
     ...
     chanNameQOX = "QO"+side +"X";
      ...
      cout <<"\nERROR! EventBuilder::DoOptimalFilterChargeX:  Argument side must be 'S1', 'S2', or empty string"
		 << endl;
      exit(1);
   }
   ...
   if(mapItr != fMapOfZipPulses.end())
   {
	 ...
	 {
	    cout <<"\nERROR! EventBuilder::DoOptimalFilterChargeX  OptimalFilterChargeX calculation not implemented for this detector type."
...
void EventBuilder::DoF5ChargeX(int detNum, const string& side)
{
   ...
   if (side == "S1" || side == "S2") {
     ...
     chanNameQOX = "QO"+side +"X";
      ...
      cout <<"\nERROR! EventBuilder::DoF5ChargeX:  Argument side must be 'S1', 'S2', or empty string"
		 << endl;
      exit(1);
   }
   ...
   if(mapItr != fMapOfZipPulses.end())
   {
            ...
            aTempPulseData->GetDetectorType() != BatRootTypes::kCDMSliteSoudanII )
	 {
	    ...
	    cout <<"\nERROR! EventBuilder::DoF5ChargeX  F5ChargeX calculation not implemented for this detector type."
	 ...
	 if(detNumQI == detNumQO && detNumQI != -999999)
	 {
	   ...
	   // set std and baseline mean
	   
	   // Revert to unnormalized std to use it with raw pulse.

	   double QIstd = aPulseDataQI->GetPulseAnalysis("BasicPulseCalc").GetRQVal("std")*
	     aPulseDataQI->GetPulseAnalysis("BasicPulseCalc").GetRQVal("norm");
	   double QOstd = aPulseDataQO->GetPulseAnalysis("BasicPulseCalc").GetRQVal("std")*
	     aPulseDataQO->GetPulseAnalysis("BasicPulseCalc").GetRQVal("norm");
	   
	   // baseline hasn't been normalized
	   double QIbs = aPulseDataQI->GetPulseAnalysis("BasicPulseCalc").GetRQVal("bs");
	   double QObs = aPulseDataQO->GetPulseAnalysis("BasicPulseCalc").GetRQVal("bs");
	   
	   tempF5ChargeX.SetQIStd(QIstd);
	   tempF5ChargeX.SetQOStd(QOstd);
	   tempF5ChargeX.SetQIBaseline(QIbs);
	   tempF5ChargeX.SetQOBaseline(QObs);
...
void EventBuilder::DoGpibTimingCalc()
{
  // GPIB (Flash Times)
...
void EventBuilder::DoIsrTimingCalc()
{
...
//  DMM  analysis
void EventBuilder::DoDmmCalc(int detNum)
{
...
void EventBuilder::DoIsrCalc(int detNum)
{
...
// Database analysis
void EventBuilder::DoDatabaseEventCalc()
{
  fDatabaseManager.StoreEventRQs(fAdminData.GetEventTime());
}

void EventBuilder::DoDatabasePulseCalc(int detNum)
{
  time_t evTime = fAdminData.GetEventTime();
  int detType = fDetectorConfigManager.GetDetectorMap()[detNum];
  fDatabaseManager.StoreDetectorRQs(evTime, detType, detNum);
...
void EventBuilder::FillVetoPulseCollection(vector<PulseData>& pulseCollection) const
{
   pulseCollection = fVectorOfVetoPulses;
   return;
}

void EventBuilder::FillNoiseMonitorPulseCollection(vector<PulseData>& pulseCollection) const
{
   pulseCollection = fVectorOfNoiseMonitorPulses;
   return;
}

//This function gives read-only access to the pulse values!
void EventBuilder::FillSingleZipPulseCollection(vector<PulseData>& pulseCollection,
...
bool EventBuilder::IsChosenType(const PulseData& aPulseData, const string& whichPulses) const
{
   bool isChosen = false;

   //Next, checking channel type 
   if(whichPulses != "chooseall" && whichPulses != "chooseveto"  &&
      whichPulses != "choosenoisemon" && 
      whichPulses != "choosezip" && whichPulses != "choosephonon"  &&
      whichPulses != "choosecharge" && whichPulses != "none")
   { 
     cout <<"EventBuilder::WARNING!  Unknown pulse collection choice: " << whichPulses
   ...
   if(whichPulses == "choosenoisemon")
   {
      if(aPulseData.IsNoiseMonitorPulse()) isChosen = true;
      return isChosen;
   }
